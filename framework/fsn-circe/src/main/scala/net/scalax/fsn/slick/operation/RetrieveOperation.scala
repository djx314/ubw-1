  ...
  def convert(
    ...
    FPile.transformTreeList { path =>
      FAtomicQuery(needAtomic[SlickRetrieve] :: needAtomicOpt[OneToOneRetrieve] :: HNil)
        .mapToOption(path) { case (slickReader :: oneToOneRetrieveOpt :: HNil, data) => {
      ...
      val readersWithData = genList.zipWithIndex.map { case (reader1, index) =>
    ...
    val results = currents.map { case (table, eachWrap) =>
      ...
      for {
        ...
        (fillCols, fillSubGens) = eachWrap.zip(retrieveData).map { case (wrap, dataItem) =>
          ...
          val subGens = wrap.reader.subGen.map { gen => new WrapTran2 {
