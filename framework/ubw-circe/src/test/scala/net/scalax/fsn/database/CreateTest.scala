...
class CreateTest extends FlatSpec
  ...
  "model" should "insert with json data" in {
    ...
    println(plan11.addOrder("abc", true).strResult.statement)
    ...
    (plan11.addOrder("abc", true).strResult.statement) shouldEqual (compare1.result.statements.toList)
    (plan11.addOrder("name", false).strResult.statement) shouldEqual (compare2.result.statements.toList)
    (plan11.addOrder("nick", false).strResult.statement) shouldEqual (compare3.result.statements.toList)
    (plan11.addOrder("nick", false).addOrders(List(ColumnOrder("name", true), ColumnOrder("abc", false))).strResult.statement) shouldEqual (compare4.result.statements.toList)
  }
