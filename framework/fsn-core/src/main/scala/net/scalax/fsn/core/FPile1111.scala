...
trait FPileList extends FPileAbs1111 {
  ...
  def encodePiles(piles: PileType): List[FPile1111]
...
case class FPileListImpl[PT, DT](
  ...
  override def encodePiles(piles: PT): List[FPile1111] = encoder(piles)
...
trait FPile1111 extends FPileAbs1111 {
  ...
  def dataFromSub(subDatas: List[Any]): DataType
...
case class FPile1111Impl[PT, DT](
    ...
    val dataFromSubFun: List[Any] => DT,
    override val subs: List[FPileAbs1111]
) extends FPile1111 {
  override type PathType = PT
  override type DataType = DT

  override def dataFromSub(subDatas: List[Any]): DataType = dataFromSubFun(subDatas)
}
