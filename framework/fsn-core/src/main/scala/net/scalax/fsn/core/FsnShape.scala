...
object FsnShape {
  ...
  implicit def hnilFsnShape[T[_]]: FsnShape[HNil, HNil, T] = new FsnShape[HNil, HNil, T] {
  ...
  implicit def fpathFsnShape[T, U[_]](implicit zeroPile: FZeroPile[U]): FsnShape[FPathImpl[T], U[T], U] = new FsnShape[FPathImpl[T], U[T], U] {
  ...
  implicit def hlistFsnShape[T <: HList, U, W, V <: HList, A[_]](
    implicit
    subShape: FsnShape[U, W, A],
    tailShape: FsnShape[T, V, A]
  ): FsnShape[U :: T, W :: V, A] = new FsnShape[U :: T, W :: V, A] {
    override def encodeColumn(pile: U :: T): List[FPath] = {
      val headPile :: hlistPile = pile
